...
<script type="text/javascript" src="../js/LineGraph.js" charset="utf-8"></script>
<script type="text/javascript">
	/****************************************************************
	 ...
	 ****************************************************************/
	var map = null;
	var proximityGraph = null;
	var lightGraph = null;
	var rotationVectorGraph = null;
	var orientationGraph = null;
	var accelerometerGraph = null;
	var linearAccelerationGraph = null;
	var magneticFieldGraph = null;
	function initGraphes() {
		map = new MapForPath("map");
		map.draw();
		proximityGraph = new LineGraph("graph8", [ "Proximity" ], ["cm"]);
		proximityGraph.draw([ [] ]);
		lightGraph = new LineGraph("graph5", [ "Light" ], ["lux"]);
		lightGraph.draw([ [] ]);
		orientationGraph = new LineGraph("graph3", [ "Azimuth", "Pitch", "Roll" ], ["°", "°", "°"]);
		orientationGraph.draw([ [] ]);
		accelerometerGraph = new LineGraph("graph1", [ "Ax", "Ay", "Az" ], ["m/s^2", "m/s^2", "m/s^2"]);
		accelerometerGraph.draw([ [] ]);
		linearAccelerationGraph = new LineGraph("graph10", [ "LAx", "LAy", "LAz" ], ["m/s^2", "m/s^2", "m/s^2"]);
		linearAccelerationGraph.draw([ [] ]);
		magneticFieldGraph = new LineGraph("graph2", [ "Mx", "My", "Mz" ], ["uT", "uT", "uT"]);
		magneticFieldGraph.draw([ [] ]);
	}
	...
	/****************************************************************
	 * Query context (JSON Format) to Servlet
	 ****************************************************************/
	function query() {
			...
			if (httpRequest.readyState == 4 && httpRequest.status == 200) {
				...
				for ( var i = 0; jsons.length; i++) {
					...
					case 1:
						...
						break;
					case 2:
						magneticFieldGraph.draw([json.value0, json.value1, json.value2]);
						break;
					case 3:
						orientationGraph.draw([json.value0, json.value1, json.value2]);
						break;
					case 5:
						lightGraph.draw([json.value0]);
						break;
					case 8:
						proximityGraph.draw([json.value0]);
						break;
					case 10:
		...
		httpRequest.open('GET',
				"/MobileContextMonitor/ContextRetriever?actionType=past&sdate="
						+ dijit.byId("sdate").value + "&stime="
						+ dijit.byId("stime").value + "&edate="
						+ dijit.byId("edate").value + "&etime="
						+ dijit.byId("etime").value, true);
		httpRequest.send();
	}
	...
	function getMovementState() {
		...
		httpRequest.onreadystatechange = function() {
			if (httpRequest.readyState == 4 && httpRequest.status == 200) {
				var states = eval('(' + this.responseText + ')');
				var state = -1;
				for(var i = 0; i < states.length; i++) {
					if(state < states[i]) state = i+1;
				}
				var stateSelector = dijit.byId("movementStateSelector");
				stateSelector.reset();
				switch(state) {
				case 1:
					dijit.byId("movementStateSelector").addOption({value: "1", label: "Staying (*)", selected: "true"});
					dijit.byId("movementStateSelector").addOption({value: "2", label: "Walking"});
					dijit.byId("movementStateSelector").addOption({value: "3", label: "Running"});
					break;
				case 2:
					dijit.byId("movementStateSelector").addOption({value: "1", label: "Staying"});
					dijit.byId("movementStateSelector").addOption({value: "2", label: "Walking (*)", selected: "true"});
					dijit.byId("movementStateSelector").addOption({value: "3", label: "Running"});
					break;
				case 3:
					dijit.byId("movementStateSelector").addOption({value: "1", label: "Staying"});
					dijit.byId("movementStateSelector").addOption({value: "2", label: "Walking"});
					dijit.byId("movementStateSelector").addOption({value: "3", label: "Running (*)", selected: "true"});
					break;
				default:
					break;
				}
		...
		httpRequest.open('GET',
				"/MobileContextMonitor/ContextReasoner?actionType=getMovementState&sdate="
				+ dijit.byId("sdate").value + "&stime="
				+ dijit.byId("stime").value + "&edate="
				+ dijit.byId("edate").value + "&etime="
				+ dijit.byId("etime").value, true);
	...
	function train() {
		...
		var state = dijit.byId("movementStateSelector").value;
		httpRequest.open('GET',
				"/MobileContextMonitor/ContextReasoner?actionType=train&state="+state+"&sdate="
				+ dijit.byId("sdate").value + "&stime="
				+ dijit.byId("stime").value + "&edate="
				+ dijit.byId("edate").value + "&etime="
				+ dijit.byId("etime").value, true);
	...
	<div id="rootContainer" dojoType="dijit.layout.BorderContainer"
		...
		<div id="toolbarContainer" dojoType="dijit.layout.ContentPane"
			...
			<div id="toolbar" dojoType="dijit.Toolbar">
				<input id="sdate" dojoType="dijit.form.DateTextBox"
					placeholder="Start Date"> <input id="stime"
                ...
                invalidMessage:'Invalid time.'"
					placeholder="Start Time"> <label>-</label> <input
					id="edate" dojoType="dijit.form.DateTextBox" placeholder="End Date">
				<input id="etime" dojoType="dijit.form.TimeTextBox"
					dojoProps="
                constraints: {
                    timePattern: 'HH:mm:ss',
                    clickableIncrement: 'T00:15:00',
                ...
                required:true,
                invalidMessage:'Invalid time.'"
					placeholder="End Time"> <input type="button" value=""
					id="queryButton" dojoType="dojox.form.BusyButton" label="Query"
					busyLabel="Querying..." timeout="2000" onClick="query()" /> <span
					dojoType="dijit.ToolbarSeparator"></span> 
					<input type="button" value="" id="getMovementStateButton"
					dojoType="dojox.form.BusyButton" label="Get Movement State"
					busyLabel="Getting..." onClick="getMovementState()" timeout="1000" />
					<!-- <select
					name="movementStateSelector" id="movementStateSelector"
					dojoType="dijit.form.FilteringSelect" placeholder="Movement State"
					required="false"> -->
					<select
					name="movementStateSelector" id="movementStateSelector"
					dojoType="dijit.form.Select" 
					placeholder="Movement State" required="false"
					>
				</select> <input type="button" value="" id="trainButton"
					dojoType="dojox.form.BusyButton" label="Train"
					busyLabel="Training..." onClick="train()" timeout="1000" />
			</div>
		</div>
		<div id="mainContainer" dojoType="dijit.layout.BorderContainer"
			...
			<div id="contentContainer" dojoType="dijit.layout.ContentPane"
				...
				<div dojoType="dojox.layout.GridContainer" id="content"
					hasResizableColumns="false" opacity="0.3" nbZones="2"
					...
					<div dojoType="dojox.widget.Portlet" title="Accelerometer"
						id="container1">
						<canvas id="graph1" width="100%" height="150px"></canvas>
					</div>
					<div dojoType="dojox.widget.Portlet" title="Linear Acceleration"
						id="container10">
						<canvas id="graph10" width="100%" height="150px">Wait...</canvas>
					</div>
					<div dojoType="dojox.widget.Portlet" title="Orientation"
						id="container3">
						<canvas id="graph3" width="100%" height="150px">Wait...</canvas>
					</div>
					<div dojoType="dojox.widget.Portlet" title="Magnetic Field"
						id="container2">
						<canvas id="graph2" width="100%" height="150px">Wait...</canvas>
					</div>
					...
					<div dojoType="dojox.widget.Portlet" title="Light" id="container5">
						<canvas id="graph5" width="100%" height="150px">Wait...</canvas>
					</div>
					<div dojoType="dojox.widget.Portlet" title="Proximity"
						id="container8">
						<canvas id="graph8" width="100%" height="150px">Wait...</canvas>
					</div>
