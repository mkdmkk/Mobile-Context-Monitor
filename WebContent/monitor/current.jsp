	...
	function getContext() {
		...
		httpRequest.onreadystatechange = function() {
			if (httpRequest.readyState == 4 && httpRequest.status == 200) {
				...
				var type = 0;
				for(type in json) {
					var context = json[type];
					//alert(type+" "+JSON.stringify(context));
					
					switch (type) {
					case "100":
						map.draw(context.values[0], context.values[1]);
						break;
					case "1":
						accelerometerGraph.draw(context.values[0], context.values[1],
								context.values[2]);
						break;
					case "10":
						linearAccelerationGraph.draw(context.values[0], context.values[1],
								context.values[2]);
						break;
					case "5":
						lightGraph.draw(context.values[0]);
						break;
					case "8":
						proximityFigure.draw(context.values[0]);
						break;
					case "11":
						break;
					case "3":
						orientationCube.draw(context.values[0], context.values[1],
								context.values[2]);
						compass.draw(context.values[0]);
						break;
					default:
						break;
					}
		...
		httpRequest.open('GET', "/MobileContextMonitor/ContextRetriever?actionType=current", true);
		httpRequest.send();
	}
